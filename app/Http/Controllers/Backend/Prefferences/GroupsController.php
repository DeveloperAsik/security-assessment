...
use App\Models\Tables\Core\Tbl_user_a_groups;
...
class GroupsController extends Controller {
    ...
    public function create(Request $request) {
        ...
        $_breadcrumbs = [
            ...
            [
                ...
                'icon' => '',
                'arrow' => true,
                'path' => config('app.base_extraweb_uri') . '/prefferences/group/view'
            ],
            [
                'id' => 3,
    ...
    public function edit(Request $request, $pr_id = null) {
        ...
        $project_types = Tbl_a_project_types::get_by_id($id);
        ...
        if (isset($request) && !empty($request)) {
            ...
            if (isset($search) && !empty($search)) {
                $data = DB::table('tbl_a_groups AS a')
                        ...
                        ->limit($limit)
                        ->get();
                $total_rows = DB::table('tbl_a_groups AS a')
                                ->where('a.id', '=', $search)
            ...
            if (isset($data) && !empty($data)) {
                ...
                foreach ($data AS $keyword => $value) {
                    ...
                    $arr[] = [
                        ...
                        <button type="button" class="btn btn-info"><a href="' . config('app.base_extraweb_uri') . '/prefferences/group/type/edit/' . base64_encode($value->id) . '" style="color:#fff;font-size:14px;" title="Edit"><i class="fas fa-edit"></i></a></button>
                        <button type="button" class="btn btn-info"><a href="' . config('app.base_extraweb_uri') . '/prefferences/group/type/remove/' . base64_encode($value->id) . '" style="color:#fff;font-size:14px;" title="Remove"><i class="fas fa-xmark"></i></a></button>
                        <button type="button" class="btn btn-info"><a href="' . config('app.base_extraweb_uri') . '/prefferences/group/type/delete/' . base64_encode($value->id) . '" style="color:#fff;font-size:14px;" title="Add"><i class="far fa-trash-alt"></i></a></button>
                      </div>',
                    ];
                }
                $output = array(
