<script src="{{config('app.base_assets_uri')}}/js/gamelib.js"></script>
<script>
var gameInitMatchingPhotos = function () {
    setgame("800x600");
            ...
            for (var j = 0; j < game.kartuY; j++) {
                ...
                if (game.mouseDitekan && game.kartuTerbuka < 2) {
                    if (cekHit(game.mouse, card) && card.stat == 0) {
                        card.stat = 2;
                        game.kartuTerbuka++;
                        if (game.kartuTerbuka == 1)
                            game.kartu1 = card;
                        if (game.kartuTerbuka == 2)
                            game.kartu2 = card;
                        card.buka = 1;
                        card.skalaX = 1;
...
var matching_photosJS = function () {
    return {
        //main function to initiate the module
        init: function () {
            gameInitMatchingPhotos();
        }
    };
}();
jQuery(document).ready(function () {
    matching_photosJS.init();
});
</script>